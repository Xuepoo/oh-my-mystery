...
也可以加入 OMM QQ 群 `1065814686` 参与讨论。

### 爬虫与数据贡献

本地协作时，爬虫项目位于相邻目录 `../mystery-clawer`。当前没有可确认的公开 GitHub URL，因此贡献者应使用本地目录和项目内文档，不要自行填写一个公共链接。爬虫层负责来源适配、原始响应缓存和规范化数据库；OMM 层通过 `scripts/build-d1-db.ts` 将爬虫数据库转换为 OMM 专用 SQLite/D1 结构，包含筛选、清理、实体合并、搜索索引和派生推荐。

两类数据库不可互相替代：爬虫数据库是采集数据的权威输入，OMM SQLite/D1 是应用使用的转换快照。修改来源适配器、事实模型或采集规则时，请同时说明来源许可、robots.txt、请求速率、缓存策略和可复现的数据库/QA 时间点。发布快照的下载、校验和解压流程见 [`docs/data-releases.md`](docs/data-releases.md)。
