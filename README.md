...
爬虫数据目前由项目维护流程持续整理。采集管线位于本地相邻目录 `../mystery-clawer`，这是协作时使用的本地项目概念链接；当前没有可确认的公开 GitHub 地址，因此不要把它当作公共仓库链接。爬虫数据库与 OMM 数据不是同一个快照：`mystery-clawer/data/mystery.db` 是采集层数据库，`scripts/build-d1-db.ts` 会筛选、清理、合并实体并生成 `omm/data/omm-d1.sqlite`（仓库内路径 `data/omm-d1.sqlite`），再用于生产 Cloudflare D1（`omm-db`）和公开发布快照。
